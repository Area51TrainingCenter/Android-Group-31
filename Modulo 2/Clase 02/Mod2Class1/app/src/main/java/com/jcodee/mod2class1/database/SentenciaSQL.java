...
public class SentenciaSQL {
    ...
    public ArrayList<Datos> obtener() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                datos.setTextoDos(cursor.getString(cursor.getColumnIndex("texto_dos")));
    ...
    public Datos obtenerDatoPorId(int id) {
        SQLiteDatabase db = conexion.getReadableDatabase();
        Cursor cursor = db.rawQuery("select * from tb_datos where id=" + id, null);
        if (cursor.moveToFirst()) {
            do {
                Datos datos = new Datos();
                datos.setId(id);
    ...
    public ArrayList<Datos> filtrar(String texto) {
        SQLiteDatabase db = conexion.getReadableDatabase();
        Cursor cursor = db.rawQuery(
                "select * from tb_datos where texto_uno like '%' || '" + texto + "' || '%'",
                null);
        ArrayList<Datos> lista = new ArrayList<>();
        if (cursor.moveToFirst()) {
            do {
